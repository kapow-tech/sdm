...
// json-annotated string fields emit datatypes.JSON (raw bytes, opaque). Message-
// kind fields emit *MessageName (typed pointer) so the View struct surfaces a
// decoded proto on read — GORM marshals/unmarshals via the protojson serializer
// registered in sdm_helpers.go (tag: serializer:protojson).
