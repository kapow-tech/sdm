...
func generateRepo(gen *protogen.Plugin, file *protogen.File) {
	...
	// `time` is referenced by Update (for `"updated_at": time.Now()`) on every
	// PII-backed message, and by chain-side Timestamp serialization
	// (model.X.AsTime().Format(time.RFC3339Nano)).
	needTime := fileHasChainTimestampField(file) || fileHasPiiBackedMessage(file)
	...
	for _, msg := range file.Messages {
		...
		// ── Upsert ────────────────────────────────────────────────────────────
		// Inserts a PII row if missing, or overwrites the listed columns on
		// existing rows (ON CONFLICT … DO UPDATE). Always appends a new chain
		// version row per chain-stored field — chain history is never rewritten.
		// Conflict target is the chain identifier key (always unique: either PK,
		// chain_identifier_key + unique, or both).
		//
		// Chain-only messages have no PII; Upsert is equivalent to SaveChain.
		...
		// ── Update ────────────────────────────────────────────────────────────
		// Updates an existing PII row's mutable columns (chain key / PK / auto-
		// increment fields excluded) and always appends a new chain version per
		// chain-stored field. Returns gorm.ErrRecordNotFound when no PII row
		// matches the chain identifier (or, for chain-only messages, no chain
		// row exists for the key).
		g.P("func (r *", modelName, "Repo) Update(ctx context.Context, model *", modelName, ") error {")
		g.P("  return r.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {")
		if !chainOnly {
			updatableCols := updatableSqlCols(msg)
			updatableFields := updatableGoFields(msg)
			conflictCol := string(keyFields[0].Desc.Name())
			conflictGoName := keyFields[0].GoName
			g.P("    updates := ", modelName, "Pii{")
			for _, f := range updatableFields {
				opts := getFieldOptions(f)
				switch {
				case needsProtojsonMarshal(f):
					g.P("      ", f.GoName, ": model.", f.GoName, ",")
				case isProtoTimestamp(f):
					g.P("      ", f.GoName, ": model.", f.GoName, ".AsTime(),")
				case opts.Json:
					g.P("      ", f.GoName, ": datatypes.JSON(model.", f.GoName, "),")
				default:
					g.P("      ", f.GoName, ": model.", f.GoName, ",")
				}
			}
			g.P("      UpdatedAt: time.Now(),")
			g.P("    }")
			g.P("    result := tx.Model(&", modelName, "Pii{}).Where(\"", conflictCol, " = ?\", model.", conflictGoName, ").")
			g.P("      Select(\"", strings.Join(updatableCols, "\", \""), "\").")
			g.P("      Updates(updates)")
			g.P("    if result.Error != nil { return result.Error }")
			g.P("    if result.RowsAffected == 0 { return gorm.ErrRecordNotFound }")
			g.P()
		} else {
			// Chain-only Update — verify a prior chain entry exists for this key.
			// Chain is append-only, so "update" semantically means "append new
			// versions, but only for records that exist." Diverges from Upsert,
			// which appends regardless.
			g.P("    var existCount int64")
			g.P("    if err := tx.Model(&", modelName, "Chain{}).Where(\"key = ?\", ", compositeKeyExpr, ").Limit(1).Count(&existCount).Error; err != nil { return err }")
			g.P("    if existCount == 0 { return gorm.ErrRecordNotFound }")
			g.P()
		}
		g.P("    // Save Chain Fields")
		emitChainEntries()
...
// fileHasPiiBackedMessage reports whether any recorded message in the file has
// a PII table (i.e. is not chain-only). PII-backed messages generate Update —
// which needs time.Now() for the updated_at column — so the repo file must
// import "time".
